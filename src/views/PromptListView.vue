    ...
    <Teleport to="body">
      <div v-if="editorOpen" class="modal-overlay" @click.self="closeEditor">
        <div class="modal-card editor-modal">
          ...
          <textarea v-model="form.content" class="textarea" placeholder="Prompt 正文..." rows="8"></textarea>
          <div class="params-row">
            <input v-model="form.sampler" class="input-sm" placeholder="Sampler" />
            <input v-model="form.steps" class="input-sm" placeholder="Steps" />
            <input v-model="form.cfg" class="input-sm" placeholder="CFG" />
            <input v-model="form.seed" class="input-sm" placeholder="Seed" />
          </div>
...
const form = ref({ title: "", content: "", sampler: "", steps: "", cfg: "", seed: "" });
...
async function loadMore() {
  ...
  try {
    const data = await apiFetch("/api/prompts?limit=20&offset=" + posts.value.length);
...
function openEditor(post) {
  if (post) {
    ...
    form.value = { title: post.title, content: post.content || "", sampler: "", steps: "", cfg: "", seed: "" };
  } else {
    editingId.value = null;
    form.value = { title: "", content: "", sampler: "", steps: "", cfg: "", seed: "" };
  }
  formImages.value = [];
  formError.value = "";
...
async function save() {
  if (!form.value.title.trim() || !form.value.content.trim()) {
    formError.value = "标题和 Prompt 不能为空";
    return;
  }
  saving.value = true;
  formError.value = "";
  ...
  try {
    ...
    const sampler = form.value.sampler.trim();
    const steps = form.value.steps.trim();
    const cfg = form.value.cfg.trim();
    const seed = form.value.seed.trim();
    ...
    if (editingId.value) {
      ...
      await apiFetch("/api/admin/prompts/" + editingId.value, {
        method: "PATCH",
      ...
      const r = await apiFetch("/api/admin/prompts", {
        method: "POST",
        body: JSON.stringify(body),
        silent: true,
      });
...
.btn-new { padding: 8px 20px; border-radius: 20px; border: 1px solid var(--glass-border); background: var(--glass-bg); color: var(--text); cursor: pointer; font-size: 13px; letter-spacing: 1px; transition: background 0.3s; }
.btn-new:hover { background: var(--glass-border); }
.post-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; max-width: 960px; margin: 0 auto; padding: 0 24px; }
...
.post-card { border-radius: 16px; overflow: hidden; background: var(--glass-bg); border: 1px solid var(--glass-border); cursor: pointer; transition: transform 0.3s, box-shadow 0.3s; position: relative; }
...
.card-prompt { font-size: 11px; font-family: monospace; opacity: 0.6; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; margin-bottom: 6px; line-height: 1.4; }
.card-meta { font-size: 12px; opacity: 0.4; }
.select-dot { position: absolute; top: 8px; right: 8px; width: 22px; height: 22px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.4); background: transparent; cursor: pointer; z-index: 10; }
.select-dot.on { background: var(--text); border-color: var(--text); }
.batch-bar { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); z-index: 100; }
.btn-outline { padding: 10px 24px; border-radius: 20px; border: 1px solid var(--glass-border); background: var(--glass-bg); backdrop-filter: blur(8px); color: var(--text); cursor: pointer; font-size: 13px; letter-spacing: 1px; }
...
.modal-card { background: var(--glass-bg); backdrop-filter: blur(16px); border: 1px solid var(--glass-border); border-radius: 16px; padding: 32px; max-width: 520px; width: 90%; }
.editor-modal { max-width: 560px; }
.modal-card h3 { font-size: 20px; font-weight: 500; margin-bottom: 20px; }
.input, .textarea { width: 100%; background: rgba(255,255,255,0.05); border: 1px solid var(--glass-border); border-radius: 10px; padding: 10px 14px; color: var(--text); font-size: 14px; outline: none; margin-bottom: 12px; font-family: inherit; }
