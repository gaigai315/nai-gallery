<template>
  <div class="view-container active">
        ...
        <tbody>
          <tr v-for="u in users" :key="u.discord_id">
            ...
            <td class="action-cell">
              ...
              <button v-if="!u.is_banned" class="btn-outline small btn-danger" @click="banUser(u.discord_id, u.username)">拉黑</button>
    ...
    <div v-if="banDialog" class="modal-overlay" @click.self="banDialog = false">
      <div class="modal-card">
        <h3>拉黑用户</h3>
        <p class="modal-hint">即使该用户从未登录过也可以拉黑。</p>
        <label>Discord ID <input v-model="banDiscordId" placeholder="用户 Discord ID" class="field-input" /></label>
        <label>原因（可选） <input v-model="banReason" placeholder="拉黑原因" class="field-input" /></label>
        <div class="modal-actions">
          <button class="btn" @click="banUser()">确定拉黑</button>
...
async function unbanUser(discordId, username) {
  if (!confirm('确定解除 ' + (username || discordId) + ' 的拉黑？')) return;
  try {
